...
switch (location.hostname) {
  ...
  case "redhat.local":
    /* falls through */
  case "local.redhat.com":
    /* falls through */
  case "qa.engage.redhat.com":
    /* falls through */
  case "cms-300.usersys.redhat.com":
    /* falls through */
  case "dynamicform-theharris.rhcloud.com":
    /* falls through */
  case "web-dev-drupal-nharris.devlab.redhat.com":
    /* falls through */
  case "www.int.openshift.com":
    //("DynamicForm Starting with SANDBOX settings.");
    elqSiteID = '711611696';
    break;
  default:
    //("DynamicForm Starting with PRODUCTION settings.");
    elqSiteID = '1795';
    break;
}
...
if (location.protocol == 'https:') {
  elqCurESite = 'https://' + elqSecERoot;
  elqAltESite = 'http://' + elqERoot;
} else {
  elqCurESite = 'http://' + elqERoot;
  elqAltESite = 'https://' + elqSecERoot;
}
...
function elqWrite() {
  if ((elqWCount > 75) || elqFail) {
    elqClearT();
  } else {
    elqWCount++;
    if (!elqLoad) {
      elqTRun = true;
      elqTID = setTimeout('elqInit();', 1000);
    } else {
      elqClearT();
      if (!elqFail) {
