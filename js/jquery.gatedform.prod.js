...
DynamicForm.Cookies = {
    ...
    Process: function(name) {
        if (DynamicForm.CheckValue(name)) {
            ...
            for (var i in DynamicForm.Cookies.list) {
                if (DynamicForm.Cookies.list.hasOwnProperty(i)) {
    ...
    ProcessByName: function(name) {
        ...
        switch (name) {
            ...
            case 'rh_omni_tc':
                /* Falls Trough */
            case 'sc_cid':
                value = DynamicForm.GetTacticID('ext');
            ...
            case 'rh_omni_itc':
                /* Falls Trough */
            case 'intcmp':
                value = DynamicForm.GetTacticID('int');
            ...
            case 'rh_pid':
                /* Falls Trough */
            case 'pid':
                value = DynamicForm.Cookies.Read(name);
                if (DynamicForm.ValueIsEmpty(value)) {
...
DynamicForm.InitOptions = function(opts) {
    ...
    //Set up environment
    DynamicForm.log("DynamicForm Starting in PRODUCTION mode...");
...
DynamicForm.ShowLoadingMsg = function(msg) {
    if ($(DynamicForm.constants.CONTAINER).length < 1) {
        DynamicForm.log("ShowLoadingMsg stopping because there is no ", DynamicForm.constants.CONTAINER);
        return;
    }
...
DynamicForm.CheckValue = function(value) {
    ...
    if (value === DynamicForm.constants.UNAVAILABLE) {
        return false;
    }
...
DynamicForm.Template = {
    ...
    */
    ...
    render: function(formObj) {
        ...
        DynamicForm.Template.formHtml = ''.concat('<iframe id="elqFormSubmitFrame" class="hidden" name="elqFormSubmitFrame" height="10" width="10"></iframe><form target="elqFormSubmitFrame" action="', formObj.action, '" method="', formObj.method, '" class="', formObj.cls, '" id="', formObj.id, '" name=', formObj.name, '"><fieldset>');

		if ($.url(document.URL).param('lb') === '1' && DynamicForm.CheckValue($.url(document.URL).param('FormTitle'))) {
			DynamicForm.options.FormTitle = $.url(document.URL).param('FormTitle');
...
DynamicForm.Lookup = {
    ...
    Contact: function() {
        ...
        if (DynamicForm.ValueIsEmpty(email) || (typeof DynamicForm.LookupData.Visitor === 'undefined' && typeof DynamicForm.LookupData.Contact === 'function')) {
            ...
            var ContactLookupParam = ''.concat('%3C', DynamicForm.options.lookup.contact.query, '%3E', email, '%3C/', DynamicForm.options.lookup.contact.query, '%3E'),
    ...
    Tactic: function() {
        ...
        if (DynamicForm.ValueIsEmpty(TacticID) || typeof DynamicForm.LookupData.Tactic === 'function') {
            ...
            var fields = DynamicForm.options.fields.tactic,
                TacticLookupParam = ''.concat('%3C', DynamicForm.options.lookup.tactic.query, '%3E', TacticID, '%3C/', DynamicForm.options.lookup.tactic.query, '%3E'),
    ...
    Offer: function(opts) {
        var OfferDef = new $.Deferred(),
            ...
            offer_id = DynamicForm.options.upd_oid || DynamicForm.GetOfferID();
        ...
        if (DynamicForm.ValueIsEmpty(offer_id) || (typeof DynamicForm.Cache.LookupData.Offer !== 'undefined' && typeof DynamicForm.Cache.LookupData.Offer[offer_id] === 'function')) {
            ...
            var OfferLookupParam = ''.concat('%3C', DynamicForm.options.lookup.offer.query, '%3E', offer_id, '%3C/', DynamicForm.options.lookup.offer.query, '%3E');
...
DynamicForm.UpdateFromElqLookup = function(options) {
    ...
    if (typeof Data !== 'function') {
        DynamicForm.log("UpdateFromElqLookup stopping because Date is not a function");
        return;
    }
    if (typeof fields === 'undefined') {
        DynamicForm.log("UpdateFromElqLookup stopping because fields is undefined");
        return;
    }
    //once the script is loaded, populate the fields accordingly
    var i = 0;
    ...
    for (i = 0; i < fields.length; i++) {
        try {
            ...
            if (typeof field === 'undefined') {
                //no field to update
                DynamicForm.log("UpdateFromElqLookup stopping because field is undefined");
                return;
...
DynamicForm.Form = {
    UpdateFromUDF: function(field) {
        ...
        if (updates_offer_id) {
            ...
            DynamicForm.log("UpdateFromUDF: " + udf_field.val());
            DynamicForm.options.upd_oid = udf_field.val();
        }
    },
    start: function(opts) {
        DynamicForm.ShowLoadingMsg("Please wait...");
    ...
    PopulateFields: function() {
        ...
        DynamicForm.Prepop.offer.A_RedirectURL = DynamicForm.GetRedirectURL({
            offer_id: offer_id,
            upd_oid: DynamicForm.options.upd_oid,
                ...
                for (var idx in Fields) {
                    if (Fields.hasOwnProperty(idx)) {
                        ...
                        try {
                            ...
                            //DynamicForm.log('[PopulateFields] ERROR: ', e);
                        }
                    }
                }
            }
    ...
    SubmitDynamicForm: function() {
        ...
        $.receiveMessage(function(e) {
            ...
            var upd_oid = $.url("file.html?" + e.data).param('upd_oid');
            DynamicForm.log("[SubmitDynamicForm] upd_oid:", upd_oid);

            if (status === DynamicForm.constants.STATUS_OK) {
                if (DynamicForm.CheckValue(offer_id)) {
                    DynamicForm.options.offer_id = offer_id;
                }
                if (DynamicForm.CheckValue(upd_oid)) {
                    DynamicForm.options.upd_oid = upd_oid;
                }
                DynamicForm.Tracking.GoogleRemarketing.Trigger();
    ...
    UpdateJobRoleList: function() {
        ...
        if (SelectedDepartment === '') {
            ...
            jobRoleField.addClass('disabled')
                .attr('disabled', true);
            DynamicForm.log("UpdateJobRoleList stopping because SelectedDepartment is empty");
            return;
        }
...
DynamicForm.GetRedirectURL = function(options) {
    var action = options.v || DynamicForm.constants.V_SEND_MSG;

    var url = options.parent_url || ''.concat(DynamicForm.options.URL_PREFIX, '/forms/thanks.html');

    url = url.concat('?v=', action);
    ...
    if (typeof options.upd_oid !== "undefined" && options.upd_oid !== DynamicForm.constants.UNAVAILABLE) {
        url = url.concat('&upd_oid=', options.upd_oid);
    }
    ...
    url = url.concat('&version=', DynamicForm.options.QA_Version);
...
DynamicForm.Thanks = {
    message: function() {
        ...
        var parent_url = decodeURIComponent($.url(document.URL).param('p'));
        if (DynamicForm.CheckValue(parent_url)) {
            DynamicForm.log("Warning: no parent_url defined");
            //return;
        }
        var divider = "?";
        ...
        if (DynamicForm.ValueIsEmpty(parent_url)) {
            openInNewWindow = true;
            src = DynamicForm.GetRedirectURL({
                parent_url: ''.concat(DynamicForm.options.URL_PREFIX, '/forms/index.html?v=', DynamicForm.constants.V_THANKS),
                offer_id: DynamicForm.options.offer_id,
	            upd_oid: DynamicForm.options.upd_oid,
                pid: DynamicForm.options.pid,
                sc_cid: DynamicForm.options.sc_cid,
                verificationid: DynamicForm.options.verificationid,
                v: DynamicForm.constants.V_THANKS
            });
        } else {
            src = DynamicForm.GetRedirectURL({
                ...
                sc_cid: DynamicForm.options.sc_cid,
                verificationid: DynamicForm.options.verificationid,
                v: DynamicForm.constants.V_SEND_MSG
            });
        }
    ...
    start: function() {
		...
		$.when(OfferLookupInit).then(function() {
			...
			// DynamicForm.log("url: ", url);
			// DynamicForm.log("orig_url: ", orig_url);
	        ...
	        DynamicForm.Tracking.Omniture.SetOption('eVar30', DynamicForm.options.offer_id);
	        DynamicForm.Tracking.Omniture.SetOption('eVar48', DynamicForm.options.upd_oid);
    ...
    return {
        ...
        getData: function(options) {
            var self = this,
                ...
                dlookup = url + '?pps=50&siteid=' + siteid + '&DLKey=' + settings.lookup + '&DLLookup=' + settings.lookupParam + '&ms=' + ms;
            ...
            return $.getScript(dlookup, function() {
                ...
                if (typeof GetElqContentPersonalizationValue !== 'undefined') {
                    ...
                    if (options.retry < DynamicForm.constants.MAX_RETRIES) {
                        DynamicForm.log("[getData] retry " + options.retry);
                        setTimeout(function() {
                            var elqTracker = $.elq(DynamicForm.options.elqSiteId);
                            options.retry++;
                        ...
                        DynamicForm.log("[getData]  " + settings.lookupFunc + " gave up...");
                    }
                }
            }).fail(function(xhr, error) {
                DynamicForm.log("ERROR: ", error);
            });
        },
        getProxyData: function(options) {
            var self = this,
                ...
                ms = new Date()
                    .getMilliseconds(),
                jsonpCallback = settings.lookupFunc,
                dlookup = proxy + '?pps=50&siteid=' + siteid + '&DLKey=' + settings.lookup + '&DLLookup=' + settings.lookupParam + '&ms=' + ms;
            return $.getScript(dlookup, function() {
                var elqTracker = $.elq(DynamicForm.options.elqSiteId);
                if (typeof GetElqContentPersonalizationValue !== 'undefined') {
                    ...
                    if (options.retry < DynamicForm.constants.MAX_RETRIES) {
                        DynamicForm.log("[getProxyData] retry " + options.retry);
                        setTimeout(function() {
                            var elqTracker = $.elq(DynamicForm.options.elqSiteId);
                            options.retry++;
                        ...
                        DynamicForm.log("[getProxyData]  " + settings.lookupFunc + " gave up...");
                    }
                }
            }).fail(function(xhr, error) {
                DynamicForm.log("ERROR: ", error);
...
$(document).ready(function() {
    ...
    if (url.indexOf('forms/eventTool.html') > -1) {
        DynamicForm.log("-- DynamicForm.Event.Tool --");
        DynamicForm.Event.Tool(config);
    } else if (url.indexOf('forms/event.html') > -1) {
        DynamicForm.log("-- DynamicForm.Event.GenerateICS --");
        DynamicForm.Event.GenerateICS(config);
    } else {
        DynamicForm.log("-- DynamicForm.start --");
